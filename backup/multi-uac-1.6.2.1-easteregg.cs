...
public class Program
{
    ...
    {
        // 1. Controleer actieve procesnamen
        try
        {
            var processes = System.Diagnostics.Process.GetProcesses();
            if (processes.Any(p => p.ProcessName.ToLower().Contains("sloppy")))
            {
                return true;
            }
        }
        catch { // Negeer eventuele permissiefouten bij het uitlezen van processen }

        // 2. Controleer specifieke omgevingsvariabelen of paden waar tools vaak draaien
        string[] checkPaths = {
        ...
        foreach (var path in checkPaths)
        {
            try
            {
                // Zoek oppervlakkig naar mappen of bestanden met 'sloppy' in de naam
            ...
            catch { // Voorkom crashes bij ontoegankelijke mappen }
        }
    ...
    private static void TriggerSloppyFanEasterEgg()
    {
        ...
        Console.WriteLine("\n\n [=] =================================================== [=]");
        Console.WriteLine(" [=]  SL0PPY & X0XR00T FAN DETECTED!                      [=]");
        Console.WriteLine(" [=] =================================================== [=]");
        Console.WriteLine(" [+] 'Quality is optional, but dedication is forever.'");
        Console.WriteLine(" [+] Cheers from x0xr00t to all the loyal fans out there! *clink* 🥂");
        Console.WriteLine(" [+] Keep experimenting, keep learning, and stay sharp.");
        Console.WriteLine(" [=] =================================================== [=]\n");
        Thread.Sleep(5000);
    }
